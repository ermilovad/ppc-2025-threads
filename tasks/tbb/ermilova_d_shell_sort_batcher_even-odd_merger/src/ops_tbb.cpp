#include "tbb/ermilova_d_shell_sort_batcher_even-odd_merger/include/ops_tbb.hpp"
...
void ParallelShellSortWithBatcherMerge(std::vector<int> &data) {
  ...
  tbb::task_arena arena(threads_count);
  arena.execute([&] {
    tbb::parallel_for(0, threads_count, [&](int thread_number) {
    ...
    for (size_t merge_size = block_size; merge_size < elements_count; merge_size *= 2) {
      ...
      tbb::parallel_for(size_t(0), merge_jobs.size(), [&](size_t j) {
